...
exports.run = exports.convertModelProps = void 0;
...
async function modelFaceToModelProp(face) {
    ...
    const matrix3d = matrix_1.Matrix.scale(4, [-1 / 16, -1 / 16, -1 / 16])
        .matmul(matrix_1.Matrix.translation(4, [(scale - 1) * 8, (scale - 1) * 8, -8]))
        .matmul(face.matrix)
        .matmul(matrix_1.Matrix.translation(4, [15 / 32, 15 / 32, 0]))
        .matmul(matrix_1.Matrix.scale(4, [1 / scale, 1 / scale, 1 / scale]))
        .t().value;
    //uvで切り取った画像を保存
    const texture = await (0, crop_1.crop)(face.texture, face.uv);
    // 面の法線方向を取得
...
async function convertModelProps(model) {
    return Promise.all((0, solver_1.resolveModelFaces)(model).map(modelFaceToModelProp));
}
exports.convertModelProps = convertModelProps;
const model2 = {
    parent: 'block/block',
    textures: {
...
async function run() {
    ...
    (0, fs_1.writeFileSync)('./result.json', JSON.stringify(result));
}
exports.run = run;
run();
